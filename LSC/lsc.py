...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
	...
	def __init__(self,app):
		...
		self.vgrid=self.create_vgrid(maxdepth=np.ceil(self.gr.mesh.nodes[:,2].max()),hsm=self.get_hsm(self.hsm.text()))
	...
	def create_transect(self,x0=[],y0=[]):
		...
		N=griddata((x,y),np.arange(0,len(self.gr.mesh.nodes[:,2]),1),(X,Y),method='nearest')
	...
	def draw_map(self,nval=60,Zmin=[],Zmax=[]):
		...
		tricon=self.axes.tricontourf(self.triangle,Z,vmin=Zmin,vmax=Zmax,cmap=plt.cm.Spectral_r,levels=levels,gid='map')
		if any(self.quad):
	...
	def refresh_it_all(self):
		...
		self.vgrid.compute_zcor(self.gr.mesh.nodes[:,2],a_vqs0=a_vqs0,dz_bot_min=dz_bot_min,opt=opt,rutgers=rutgers)
