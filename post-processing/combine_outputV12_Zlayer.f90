...
subroutine combine_output11(ibgn,iend,iwetdry,to_be_combined,output_prefix,output_zl)
!-------------------------------------------------------------------------------
  use netcdf
...
!  character(len=30),dimension(nfile)   :: files  !< base names (e.g. elev.61) of files to combine
  ...
  character(len=90)                  :: output_zl !dry option (0: use last-wet value; 1: junk)
 ...
 &ic3(:,:),isidenode(:,:),i23d(:),ivs(:),ndims(:),iu_id(:),idry(:),idry_e(:),idry_s(:)
  real,allocatable ::var4dmz(:,:,:),var3dmz(:,:,:),kbp(:),sigma_lcl(:,:),ztmp(:,:),zcor(:,:),ztot(:),sigma(:),cs(:),eta3(:,:),eta2(:),outeb(:,:,:), &
 ...
 &dps(:),eta2s(:),eta2e(:)
  ...
  integer :: nv_dims(1),time_dims(1),ele_dims(2),x_dims(1),y_dims(1),z_dims(1),sigma_dims(1), &
...
!  character(len=dataset_name_len) :: netcdf_var_name
  ...
  integer          :: varid,dimid
  integer          :: n,zinterp
  allocatable :: zlayer(:),Zl(:)
!  integer :: netcdf_var_dim
!  logical :: found_netcdf_var

  invalid_index = -99999
  n = count(transfer(output_zl, 'a', len(output_zl)) == ",") 
  allocate(Zl(n+1))
  allocate(zlayer(n+1))
  read(output_zl,*) Zl(1:n+1)
  n=n+1
  zinterp=n
  print*,'n=',n
  print*,'zl=',output_zl
  ...
  if(n/=0) then
    print*, 'it is outputed in Zlayer format'
    !check deplevels are positive
    do k=1,n-1
        if (Zl(k) < 0) stop 'deplevels should be positive (depths)'
    enddo
    !check deplevels are in increasing order
    do k=1,n-1
        if (Zl(k) >= Zl(k+1)) stop 'deplevels should be in increasing order'
    enddo
    !fill zlevels array
...
! Read local_to_global_0000 for global info
  ...
  &ztot(nvrt),sigma(nvrt),ztmp(nvrt,np_global),zcor(nvrt,np_global),cs(nvrt),ihot_len(0:nproc-1), &
  ...
  allocate(sigma_lcl(nvrt,np_global),kbp(np_global))
  print *,'read vgrid'
  call get_vgrid('vgrid.in',np_global,nvrt,ivcor,kz,h_s,h_c,theta_b,theta_f,ztot,sigma,sigma_lcl,kbp)
  print*, 'ivcor= ', ivcor
...
!    iret = nf90_def_dim(ncid, 'nSCHISM_vgrid_layers',nvrt, nv_dim)
    iret = nf90_def_dim(ncid, 'nz',zinterp, nv_dim)   
...
!    int_buffer(1) = 1  
    ...
    nv_dims(1)=nv_dim
    iret=nf90_def_var(ncid,'lev',NF90_FLOAT,nv_dims,inv_id)
    iret=nf90_put_att(ncid,inv_id,'long_name','Z coordinates at whole levels')
    iret=nf90_put_att(ncid,inv_id,'units','meters')
    iret=nf90_put_att(ncid,inv_id,'positive','up')
...
!    data_count_1d(1)=ne_global
    ...
    iret=nf_put_vara_real(ncid,inv_id,1,zinterp,zlayer)  
...
!        endif !i23d
        ...
        if (variable_nm(m)=='elev') then         
          if (allocated(eta2)) then
             deallocate(eta2)
          endif
          allocate(eta2(np_global))
          eta2=outd(m)%data(1,1,1:data_count_2d(1))
        endif
        if(i23d(m)==0) then !time
          hc_array(1)=time
  ...
  !        print*, 'interpolating to zlevel',m,variable_nm(m)
          do i=1,np_global
        ...
        !         idry(i)=0            
                ...
                call zcor_SZ(dp(i),eta2(i),h0,h_s,h_c,theta_b,theta_f,kz,nvrt,ztot,sigma,ztmp(:,i),idry(i),kbpl)
                kbp(i)=1
             endif
           enddo
          ...
          if(ivs(m)==1) then
            data_start_3d(1:2)=1; data_start_3d(3)=ispool
            data_count_3d(1)=zinterp; data_count_3d(3)=1
            if (allocated(var3dmz)) then
               deallocate(var3dmz)
            endif
            if (allocated(var4dmz)) then
               deallocate(var4dmz)
            endif
            allocate(var3dmz(1,zinterp,data_count_3d(2)))
            allocate(var4dmz(1,nvrt,data_count_3d(2)))
            var4dmz(1,:,:)=outd(m)%data(1,:,1:data_count_3d(2))          
            call interpolatez(np_global,zinterp,var4dmz, ztmp,kbp00,zlayer,var3dmz,nvrt)
            iret=nf90_put_var(ncid,iu_id(m),var3dmz,data_start_3d,data_count_3d)
          else
            data_start_4d(1:3)=1; data_start_4d(4)=ispool
            data_count_4d(1)=2; data_count_4d(2)=zinterp; data_count_4d(4)=1
            if (allocated(var3dmz)) then
               deallocate(var3dmz)
            endif
            if (allocated(var4dmz)) then
            ...
            allocate(var3dmz(2,zinterp,data_count_3d(2)))
            allocate(var4dmz(1,nvrt,data_count_3d(2)))
            var4dmz(1,:,:)=outd(m)%data(1,:,1:data_count_4d(3))
           
            call interpolatez(np_global,zinterp,var4dmz,ztmp,kbp,zlayer,var3dmz(1,:,:),nvrt)
            var4dmz(1,:,:)=outd(m)%data(2,:,1:data_count_4d(3))
            call interpolatez(np_global,zinterp,var4dmz,ztmp,kbp,zlayer,var3dmz(2,:,:),nvrt)
            
            iret=nf90_put_var(ncid,iu_id(m),var3dmz,data_start_4d,data_count_4d)
          endif
        endif !i23d
      enddo !m=1,nvars
      iret = nf90_sync(ncid)
...
subroutine combine_output11_input(ibgn,iend,iwetdry,to_be_combined,output_prefix,output_zl)
use argparse
implicit none
...
character(len=10240),intent(out)                  :: output_zl !dry value option
...
call cla_register('-l','--lev','output level',  cla_char,'0,1')
call cla_validate
...
integer,dimension(nfilemax)          :: output_zl
call combine_output11_input(ibgn,iend,iwetdry,to_be_combined,output_prefix,output_zl)
call combine_output11(ibgn,iend,iwetdry,to_be_combined,output_prefix,output_zl)
end program

subroutine interpolatez(np,nz,varin, zin, kz, zout, varout,nvrt)
  real, intent(in)    :: varin(1,nvrt,np), zin(nvrt,np), zout(nz)
!  real, intent(in)    :: varin(:,:,:), zin(:,:), zout(:)
  integer, intent(in) :: kz(np)
  real, intent(out)   :: varout(1,nz,np)!(np,nz)
  integer             :: i,k,kout,k0
  real                :: rat
  ...
  ! for a given time instant, if a requested level is above water varout ==
  ! surface
  ! if below, varout is interpolated.

  !varout=null_real  !PROBLEM: if dry, varout will be null_real instead of 0.0
...
!  endif
  ...
  do i=1,np
    ...
    if (abs(zin(kz(i),i)) <= almost_zero .and. abs(zin(nvrt,i)) <= almost_zero) cycle ! dry nodes have all zins =0.

    do kout=1,nz

      ! interpolate z
      if (zout(kout) < zin(kz(i),i)) then ! zout below bathymetry
        ! do nothing
      elseif (zout(kout) >= zin(nvrt,i)) then
        !write(*,*) 'getting surface!'
        varout(1,kout,i) = varin(1,nvrt,i)
      else

        ! interp1d
        k0=null_int ! layer below
        do k=kz(i),nvrt-1
          if (zin(k,i) <= zout(kout) .and. zout(kout) < zin(k+1,i)) then
            k0=k
            exit
          endif
        enddo !k
        ...
        if (k0<0) then
          ! should never enter here because below and above zin cases were
          ! comtemplated before
...
!          write(*,*) "zin = ", zin(i,:)
        ...
        rat = (zout(kout)-zin(k0,i)) / (zin(k0+1,i)-zin(k0,i))
        varout(1,kout,i) = varin(1,k0,i)*(1.-rat)+varin(1,k0+1,i)*rat

      endif

      !if (debug .and. i==debug_node) then
      !  write(*,*) 'kout, zout(kout), k0, zin(i,k0), zin(i,k0+1), varin(i,k0),
      !  varin(i,k0+1), varout(i,kout) = ', &
      !              kout, zout(kout), k0, zin(i,k0), zin(i,k0+1), varin(i,k0),
      !              varin(i,k0+1), varout(i,kout)
      !endif
